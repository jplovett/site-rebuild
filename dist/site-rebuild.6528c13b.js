...
/* ---------- View injection (unchanged core) ---------- */ parcelHelpers.export(exports, "injectView", ()=>injectView);
/* ---------- App bootstrap ---------- */ parcelHelpers.export(exports, "startApp", ()=>startApp);
var _bootstrap = require("bootstrap");
var _settingsJs = require("../config/settings.js");
var _settingsJsDefault = parcelHelpers.interopDefault(_settingsJs);
var _navRoutesJs = require("../config/navRoutes.js");
var _navRoutesJsDefault = parcelHelpers.interopDefault(_navRoutesJs);
var _appJs = require("./app.js");
const snipCache = new Map();
/* ---------- Snip loading / caching ---------- */ async function fetchSnip(path) {
...
async function injectView(viewKey) {
    const route = (0, _navRoutesJsDefault.default)[viewKey] || (0, _navRoutesJsDefault.default).home;
    ...
    for (const rowObj of route.rows || []){
        ...
        const colsDefs = Array.isArray(rowValue) ? rowValue : [
            rowValue
        ];
...
/* ---------- Helpers for routing and nav state ---------- */ function normalizeRouteKeyFromHref(href) {
    try {
...
/* ---------- History API router and nav click handler ---------- */ function pushViewToHistory(viewKey, title) {
...
function onNavClick(e) {
    ...
    const title = (0, _navRoutesJsDefault.default)[viewKey] && (0, _navRoutesJsDefault.default)[viewKey].Title || viewKey;
    // Push into history and inject view
    pushViewToHistory(viewKey, title);
    injectView(viewKey);
}
/* Popstate handler to catch back/forward and inject previous view */ function onPopState() {
...
function applyMetaTags() {
    // Set document title
    document.title = (0, _settingsJsDefault.default).title;
    // Set meta description
    setOrCreateMeta('description', (0, _settingsJsDefault.default).description);
    // Set meta keywords
    setOrCreateMeta('keywords', (0, _settingsJsDefault.default).keywords);
}
...
function startApp() {
    ...
    history.replaceState({
        view: initialView
    }, (0, _navRoutesJsDefault.default)[initialView]?.Title || "", initialView === "home" ? "/" : `/${initialView}`);
    // Render the initial view
    injectView(initialView);
    (0, _appJs.buildFooter)();
    applyMetaTags();
}
...
parcelHelpers.export(exports, "buildFooter", ()=>buildFooter);
var _bootstrap = require("bootstrap");
var _settingsJs = require("../config/settings.js");
var _settingsJsDefault = parcelHelpers.interopDefault(_settingsJs);
var _navRoutesJs = require("../config/navRoutes.js");
...
const buildLeftHeader = ()=>{
    ...
    div.innerHTML = `
    <img src="${(0, _settingsJsDefault.default).thumbnail}" alt="thumb" class="brand-thumb me-2">
    <div class="brand-text">
      <div class="h5 mb-0 site-title">${(0, _settingsJsDefault.default).title}</div>
      <div class="small text-warning site-sub">${(0, _settingsJsDefault.default).subTitle}</div>
    </div>
  `;
    return div;
};
...
function buildHeader() {
    ...
    collapseDiv.className = "collapse navbar-collapse";
    // nav links
    const navLinks = buildNavLinks((0, _navRoutesJsDefault.default));
